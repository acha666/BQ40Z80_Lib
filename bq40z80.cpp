...
#include "bq40z80.h"
    ...
    uint8_t BQ40Z80::get_average_time_to_empty()
    {
        uint16_t buf;
        ESP_ERROR_CHECK(this->read_word(BQ40Z80_SBS_AverageTimeToEmpty, &buf));
        return buf;
    }

    uint8_t BQ40Z80::get_average_time_to_full()
    {
        uint16_t buf;
        ESP_ERROR_CHECK(this->read_word(BQ40Z80_SBS_AverageTimeToFull, &buf));
        return buf;
    }

    uint8_t BQ40Z80::get_cycle_count()
    {
        uint16_t buf;
        ESP_ERROR_CHECK(this->read_word(BQ40Z80_SBS_CycleCount, &buf));
        return buf;
    }

    uint8_t BQ40Z80::get_design_capacity()
    {
        uint16_t buf;
        ESP_ERROR_CHECK(this->read_word(BQ40Z80_SBS_DesignCapacity, &buf));
        return buf;
    }

    uint8_t BQ40Z80::get_design_voltage()
    {
        uint16_t buf;
        ESP_ERROR_CHECK(this->read_word(BQ40Z80_SBS_DesignVoltage, &buf));
        return buf;
    }

    uint8_t BQ40Z80::get_cell_voltage(uint8_t cell)
    {
        assert(1 <= cell && cell <= 7);
        //todo
    }
